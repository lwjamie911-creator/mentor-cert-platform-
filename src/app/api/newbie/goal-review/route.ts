...
const ALLOWED_FIELDS = ['workGoalUrl', 'month1Url', 'month2Url', 'month3Url', 'debriefUrl'] as const
type Field = (typeof ALLOWED_FIELDS)[number]

// debriefUrl（新人述职）不限地址格式；其余项要求腾讯文档链接
const UNRESTRICTED_FIELDS: readonly Field[] = ['debriefUrl']

// POST: 新人提交/更新某一项文档链接（工作目标 / 第1-3个月月报 / 新人述职）
export async function POST(req: Request) {
  ...
  // 允许传空（清空链接）；受限项非空时必须是腾讯文档链接，述职不限格式
  const trimmed = typeof url === 'string' ? url.trim() : ''
  if (trimmed && !UNRESTRICTED_FIELDS.includes(field) && !isTencentDocUrl(trimmed)) {

...
// 评分：连线题 answers[qId] = { leftIndex: rightOrigIndex }
function scoreQuestion(q: any, userAnswer: any): boolean {
  const correctAnswer: string[] = JSON.parse(q.answer)

  if (q.type === 'matching') {
    // userAnswer = { "0": 0, "1": 2, "2": 1 } (leftIndex → rightOrigIndex)
    if (!userAnswer || typeof userAnswer !== 'object') return false
    return correctAnswer.every((_, li) => Number(userAnswer[li]) === li)
  }
...
export async function POST(req: Request) {
  ...
  for (const q of questions) {
    if (scoreQuestion(q, answers[q.id])) correctCount++
  }
  ...
  if (examType === 'final' && passed && courseId) {
    ...
    if (!existing) {
      ...
      const certNo   = `TEG-${dateStr}-${String(todayCount + 1).padStart(4, '0')}`
      const expiresAt = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000 * 2)
      await prisma.certificate.create({
        data: { userId: session.user.id, courseId, certificateNo: certNo, expiresAt },
      })

          ...
          {resultQs.map((q, i) => {
            ...
            return (
              <div key={q.id} className={`bg-white rounded-2xl border p-5 ${
                correct ? 'border-green-100' : 'border-red-100'
              }`}>
                  ...
                  {q.options.map((opt, oi) => {
                    ...
                    return (
                      <div key={oi} className={`text-sm px-3 py-2 rounded-xl flex items-center gap-2 ${
                        ...
                        isSelected && !isAnswer? 'bg-red-50 text-red-700' :
                        'text-gray-500'
                      }`}>
                        <span className={`w-5 h-5 rounded-full text-xs flex items-center justify-center flex-shrink-0 font-bold ${
                          ...
                          isSelected && !isAnswer ? 'bg-red-200 text-red-600' :
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            {questions.map((q, i) => (
              <div key={q.id} className={`w-2 h-2 rounded-full transition-colors ${
                answers[q.id] ? 'bg-indigo-500' : 'bg-gray-200'
              }`} />
            ))}
          </div>
      ...
      <div className="space-y-4">
        {questions.map((q, i) => {
          ...
          const answered = !!(Array.isArray(selected) ? selected.length > 0 : selected)
          return (
            <div key={q.id} className={`bg-white rounded-2xl border p-5 transition-all ${
              answered ? 'border-indigo-200 shadow-sm' : 'border-gray-100'
            }`}>
              <div className="flex items-start gap-2 mb-4">
                <span className={`flex-shrink-0 w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center ${
                  answered ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-400'
                }`}>{i + 1}</span>
                  ...
                  return (
                    <label key={oi} className={`flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-all ${
                      isSelected
                        ...
                        : 'border border-gray-100 hover:border-indigo-200 hover:bg-indigo-50/40'
                    }`}>
                      <input
                        ...
                        value={optKey}
                        checked={isSelected}
                        onChange={() => handleAnswer(q.id, optKey, q.type)}
                        className="sr-only"
                      />
                      ...
                      <span className={`text-sm ${isSelected ? 'text-indigo-700 font-medium' : 'text-gray-600'}`}>
                        {opt}
                      </span>
                    </label>
                  )
      ...
      <div className="sticky bottom-4">
        <button
          ...
          style={{ background: allAnswered
            ? 'linear-gradient(90deg, #4f46e5, #7c3aed)'
            : undefined,
            backgroundColor: !allAnswered ? '#d1d5db' : undefined,
          }}
        >
          {submitting
            ? '提交中…'
